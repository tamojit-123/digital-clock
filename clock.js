...
function splitDigits(stringVal, array) {
    const tempString = stringVal.toString();
    array.length = 0;
...
function printDigits(array, elem) {
    for (let count = 0; count < array.length; count++) {
        for (let segmentCount = 0; segmentCount < 7; segmentCount++) {
            if (checkVal(array[count], constants[segmentCount]) === true) {
                turnOn(
                    elem + (count + 1) + " div:nth-of-type(" + (segmentCount + 1) + ")"
                );
            } else {
                turnOff(
                    elem + (count + 1) + " div:nth-of-type(" + (segmentCount + 1) + ")"
